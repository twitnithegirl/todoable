...
class TodoableListTest < Minitest::Test
  ...
  def setup
    ...
    VCR.use_cassette('sample_list') do
      @list = @client.new_list(name: 'Sample list')
    end
  end
  ...
  def test_it_makes_an_item
    ...
    VCR.use_cassette('make_an_item') do
      response = @client.new_item(list_id: @list['body']['id'], name: 'Workoutz')
  ...
  def test_it_gives_an_error_then_you_try_to_make_an_item_with_nil_name
    ...
    VCR.use_cassette('make_an_item_without_name') do
      response = @client.new_item(list_id: @list['body']['id'], name: nil)
  ...
  def test_it_marks_an_item_finished
    ...
    VCR.use_cassette('mark_item_finished') do
      item = @client.new_item(list_id: @list['body']['id'], name: 'Call ma')
      response = @client.finish_item(list_id: @list['body']['id'], item_id: item['body']['id'])
  ...
  def test_mark_item_finished_returns_an_error_code
    ...
    VCR.use_cassette('mark_item_finished_error_code') do
      item = @client.new_item(list_id: @list['body']['id'], name: 'Call pop')
      @client.finish_item(list_id: @list['body']['id'], item_id: item['body']['id'])
      response = @client.finish_item(list_id: @list['body']['id'], item_id: item['body']['id'])
      assert_equal 404, response['code']
    end
  end
  ...
  def test_it_deletes_an_item
    ...
    VCR.use_cassette('delete_an_item') do
      item = @client.new_item(list_id: @list['body']['id'], name: 'Lame todootle doot')
      response = @client.delete_item(list_id: @list['body']['id'], item_id: item['body']['id'])
  ...
  def test_it_gives_an_error_if_it_cant_find_item_to_delete
    ...
    VCR.use_cassette('delete_an_item_psych') do
      response = @client.delete_item(list_id: @list['body']['id'], item_id: 4)
      assert_equal 404, response['code']
    end
  end
end